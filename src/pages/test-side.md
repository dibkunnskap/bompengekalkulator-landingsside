...
The API has three methods which are explained below:

* GetFeesByWaypointList (most commonly used by autologs and travel apps that record waypoints on the route)
* GetFeesByWaypoints (used by some apps for travel expenses)
* FindRouteDescriptions (not very common)


# GetFeesByWaypointList

```
POST /api/bomstasjoner/GetFeesByWaypointList
```
This method is the most commonly used. You post a list of waypoints of the actual trip, and the API returns toll roads and ferries along the route.
...
This method is used when you only have To/From (and optional via waypoints). We use the Google Maps API to calculate the route. Instead of "WaypointList" you use Fra/Til/Vialiste. 
